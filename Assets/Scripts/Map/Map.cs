...
public class Map : MonoBehaviour
{
   ...
   List<List<int>> InitializeMapRoomEdges()
    ...
    {
        if (PlayDataCon.Instance.PlayData == null) //저장 데이터가 없을 경우
        {
            int currentRoomCount = 1;
            int currentRoomIndex = 0;

            List<Vector2> mapRoomPoints = new List<Vector2>(roomCount);
            List<List<int>> mapRoomEdges = InitializeMapRoomEdges();
    ...
    void PlaceMapRooms()
    {
        for (int node = 0; node < mapRooms.Count; node++)
        {
            ...
            mapRooms[node].transform.position = new Vector3(
                mapRooms[node].RoomPoint.x * (roomSize.x + roomInterval),
                0,
                mapRooms[node].RoomPoint.y * (roomSize.y + roomInterval)
            );
        }
        ...
        for (int node = 0; node < mapRooms.Count; node++)
        {
            ...
            for (int dir = 0; dir < mapRooms[node].RoomEdges.Count; dir++)
            {
                //해당 방향에 연결된 방이 있는 경우
                if (mapRooms[node].RoomEdges[dir] != -1)
                {
                    //서로를 잇는 문 추가
                    mapRooms[node].SetDoorsDictionary((Define.Direction)dir, mapRooms[mapRooms[node].RoomEdges[dir]]);
                }
            }
        }
    }
