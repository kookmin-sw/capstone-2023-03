...
{
    public int Level { get; set; } = 0; //현재 스테이지 레벨
    ...
    private void CreateMapRooms()
    {
        ...
        for (int node = 0; node < roomCount; node++)
        {
            ...
            //방 게임오브젝트 생성
            Room currentRoom = AssetLoader.Instance.Instantiate($"Prefabs/Room/Room{Level}", Map.transform).AddComponent<Room>();
        ...
        if (Level == 4) { Rooms[0].Symbol.transform.position += new Vector3(-1.5f, 0, 1.5f); }
    }
